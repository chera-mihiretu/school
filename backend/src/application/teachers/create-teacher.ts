...
export type CreateTeacher = (input: CreateTeacherInput) => Promise<CreateTeacherResult>;

export function createCreateTeacher(deps: {
  rootHost: string;
  adminEmail: string;
  ...
  return async (input) => {
    ...
    const invalidSex = validateTeacherSex(input.sex);
    if (invalidSex !== undefined) {
      return { ok: false, status: 400, error: invalidSex };
    }
    if (!isTeacherSex(input.sex)) {
    ...
    const invalidEmail = validateTeacherEmail(email);
    if (invalidEmail !== undefined) {
      return { ok: false, status: 400, error: invalidEmail };
    }

    if (teacherEmailsEqual(email, adminEmail)) {
      return { ok: false, status: 409, error: TEACHER_EMAIL_UNAVAILABLE };
    }
    const directorEmail = authorized.context.tenant.email;
    if (directorEmail !== null && teacherEmailsEqual(email, directorEmail)) {
      return { ok: false, status: 409, error: TEACHER_EMAIL_UNAVAILABLE };
    }

    const existing = await teachers.findByEmail(authorized.context.slug, email);
    ...
    const n = await teachers.nextPersonNumber(
      authorized.context.slug,
      "T",
      yearYy,
    );
    ...
    const inserted = await teachers.insert(authorized.context.slug, {
      givenName,
      fatherName,
      grandfatherName,
      sex: input.sex,
    ...
    const sent = await sendTeacherCredentials(
      { teachers, mailer, publicUrls },
      {
        ...
        teacherId: inserted.teacher.id,
        schoolName: authorized.context.tenant.name,
        email,
        password,
        schoolId: inserted.teacher.employeeId,
      },
    );
    const teacher = sent.teacher ?? inserted.teacher;
