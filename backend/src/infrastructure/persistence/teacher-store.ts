...
type TeacherAuthRow = TeacherRow & {
  password_hash: string;
};

function mapRow(row: TeacherRow): Teacher {
  if (!isTeacherSex(row.sex)) {
    throw new Error("invalid teacher sex");
  }
  return {
    id: row.id,
...
function mapAuth(row: TeacherAuthRow | undefined) {
  if (row === undefined || row.password_hash.length === 0) {
    return undefined;
  }
  return { teacher: mapRow(row), passwordHash: row.password_hash };
}
...
export function createTeacherStore(sql: Sql): TeacherStorePort {
  return {
    async ensureSchema(slug) {
      await ensureTenantTeachersTable(sql, slug);
    },
    async nextPersonNumber(slug, role, yearYy) {
      const schemaName = await ensureTenantTeachersTable(sql, slug);
    ...
    async insert(slug, input) {
      ...
      try {
        const rows = await sql<TeacherRow[]>`
          insert into ${tenantTeachersRelation(sql, schemaName)} (
            id,
            given_name,
            father_name,
            grandfather_name,
        ...
        const row = rows[0];
        if (row === undefined) {
          throw new Error("teacher insert returned no row");
        }
        return { ok: true, teacher: mapRow(row) };
      } catch (err) {
    ...
    async findAuthByEmail(slug, email) {
      try {
        ...
        const rows = await sql<TeacherAuthRow[]>`
          select
            id,
            given_name,
            father_name,
            ...
            created_at,
            password_hash
          from ${tenantTeachersRelation(sql, schemaName)}
    ...
    async updatePassword(slug, input) {
      const schemaName = await ensureTenantTeachersTable(sql, slug);
      const rows = await sql<TeacherRow[]>`
        update ${tenantTeachersRelation(sql, schemaName)}
        set
    ...
    async recordSignIn(slug, id, at) {
      const schemaName = tenantSchemaName(slug);
      try {
        const updated = await sql<TeacherRow[]>`
          update ${tenantTeachersRelation(sql, schemaName)}
          set signed_in_at = ${at}
        ...
        return { ok: true, teacher: mapRow(existing) };
      } catch (err) {
        if (isUndefinedTable(err)) {
          return { ok: false, reason: "not_found" };
        }
