...
import {
  ensureTenantStudentsTable,
  tenantIdCountersRelation,
  tenantStudentsRelation,
...
function mapRow(row: StudentRow): Student {
  if (!isStudentSex(row.sex)) {
    throw new Error("invalid student sex");
  }
  return {
    id: row.id,
...
function mapAuth(row: StudentAuthRow | undefined) {
  if (row === undefined || row.password_hash.length === 0) {
    return undefined;
  }
  return { student: mapRow(row), passwordHash: row.password_hash };
}
...
export function createStudentStore(sql: Sql): StudentStorePort {
  return {
    async ensureSchema(slug) {
      await ensureTenantStudentsTable(sql, slug);
    },
    async nextPersonNumber(slug, role, yearYy) {
      const schemaName = await ensureTenantStudentsTable(sql, slug);
    ...
    async insert(slug, input) {
      ...
      try {
        const rows = await sql<StudentRow[]>`
          insert into ${tenantStudentsRelation(sql, schemaName)} (
            id,
            given_name,
            father_name,
            grandfather_name,
        ...
        return { ok: true, student: mapRow(row) };
      } catch (err) {
    ...
    async findAuthByEmail(slug, email) {
      try {
        ...
        const rows = await sql<StudentAuthRow[]>`
          select
            id,
            given_name,
            father_name,
            ...
            created_at,
            password_hash
          from ${tenantStudentsRelation(sql, schemaName)}
    ...
    async updatePassword(slug, input) {
      const schemaName = await ensureTenantStudentsTable(sql, slug);
      const rows = await sql<StudentRow[]>`
        update ${tenantStudentsRelation(sql, schemaName)}
        set
    ...
    async recordSignIn(slug, id, at) {
      const schemaName = tenantSchemaName(slug);
      try {
        const updated = await sql<StudentRow[]>`
          update ${tenantStudentsRelation(sql, schemaName)}
          set signed_in_at = ${at}
        ...
        return { ok: true, student: mapRow(existing) };
      } catch (err) {
        if (isUndefinedTable(err)) {
          return { ok: false, reason: "not_found" };
        }
