...
export function TeacherPasswordView({
  ...
  async function submitPassword(): Promise<void> {
    ...
    setPending(true);
    try {
      const result = await changeTeacherPasswordAction(newPassword);
      if (!result.ok) {
  ...
  return (
    <FirstLoginFrame wide>
      ...
      >
        <FirstLoginFieldLabel htmlFor="teacher-new-password">
          New password
        </FirstLoginFieldLabel>
        <FirstLoginTextInput
          ...
          className="mb-8"
        />
        <FirstLoginFieldLabel htmlFor="teacher-confirm-password">
